...
class UKBB2020:
    ...
    def get_metadata(self, predefined=["demographics"], cols=[],
        ...
        # for all usecols find the resp. description, dtype and category names 
        col_descs = {}
        col_dtypes = {}   
        col_dates = []
        col_cat_remap = {}         
...
"if 'binarize' is requested then class0 and class1 cases should be specified."
    ...
    def prepare_X(self, 
        ...
        isinstance(preloaded_X, (pd.DataFrame, pd.Series))):
            ...
            self.df_h5 = self.df_h5.merge(preloaded_X, on="eid")      
            # name of output hdf5
            self.hdf5_name_x += "-{}".format("participants") 
        # if a MRI column name from BIDS csv file is provided then set this path as X 
            ...
            self.hdf5_name_x = "sMRIfeatures-{}".format(feature_filtername)
            feature = pd.read_csv(f"{self.BIDS_DIR}{self.file_name}_idp.csv",index_col="eid")
            # filter columns
            feature = feature.filter(regex=feature_cols)
            self.X_colnames = list(feature.columns)
            self.df_h5 = self.df_h5.merge(feature, on="eid")
...
# ### SAVE TO HDF5 ###
    ...
    #                     drop_useless_voxels=False
        ...
        dest = join(self.BIDS_DIR, "../h5files" + filename_prefix + ".h5")      
       ...
       # if nd == 3:
       #     data_crop = X[x_crop:-x_crop, y_crop:-y_crop, z_crop:-z_crop]
       # elif nd == 4:
       #     data_crop = X[:, x_crop:-x_crop, y_crop:-y_crop, z_crop:-z_crop]
       # else:
